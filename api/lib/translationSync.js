const { searchActivities, getActivityById, applyQuoteCurrency, getQuoteCurrency } = require('./bokun');
const { normalizeActivity, normalizeSearchResponse } = require('./normalizeActivity');
...
async function runTranslationSync(opts = {}) {
  ...
  if (activityIds && activityIds.length) {
    ...
    const raw = await searchActivities({ uiLang, page: 1, pageSize: Math.min(limit, 50) });
    const { activities: normalized } = normalizeSearchResponse(raw);
    const quoteCurrency = getQuoteCurrency();
    activities = applyQuoteCurrency(normalized.slice(0, limit), quoteCurrency);
  }
...
module.exports = { runTranslationSync, syncActivityTranslations };
