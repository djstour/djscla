...
function inferQuestionsFromActivity(activity, item, lang = 'hant', opts = {}) {
  const { skipContactQuestions = false } = opts;
  const questions = skipContactQuestions ? [] : [...baseContactQuestions(lang)];

  const hasDate = !!(item && item.date);
  const hasStartTime = !!(item && item.startTimeId);

  if ((activity.bookingType === 'DATE' || activity.bookingType === 'DATE_AND_TIME') && !hasDate) {
    questions.push({
      id: 'travel_date',
      scope: 'activity',
      type: 'date',
  ...
  if (activity.bookingType === 'DATE_AND_TIME' && !hasStartTime) {
    questions.push({
      id: 'start_time_id',
      scope: 'activity',
      type: 'options',
