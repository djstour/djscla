...
/**
 * Convert Bókun's "advanced" cancellation rules into a single user-friendly
 * "free until X hours before" cutoff. The rules ship as an ordered list of
 * { cutoffHours, charge, percentage }; we look for the smallest cutoff that
 * still charges 0% — that is the latest you can cancel without penalty.
 */
function deriveCancellationCutoff(item) {
  const policy = item.cancellationPolicy;
  if (!policy || !Array.isArray(policy.penaltyRules)) return null;
  const freeRules = policy.penaltyRules
    .filter((r) => Number(r.charge ?? r.percentage ?? 100) === 0)
    .map((r) => Number(r.cutoffHours))
    .filter((h) => Number.isFinite(h) && h > 0)
    .sort((a, b) => a - b);
  if (freeRules.length) return freeRules[0];
  return policy.simpleCutoffHours || null;
}

function buildBookableExtras(item) {
  if (!Array.isArray(item.bookableExtras)) return [];
  return item.bookableExtras.slice(0, 12).map((ex) => ({
    id: ex.id,
    title: ex.title,
