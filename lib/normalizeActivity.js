...
function buildPickupInfo(item) {
  ...
  return {
    ...
    dropoffEnabled: !!(
      item.dropoffEnabled
      || embedded?.dropoffEnabled
      || (item.meetingType && item.meetingType.dropoffService)
    ),
  };
}
...
function normalizeActivity(item) {
  ...
  const base = {
    ...
    privateExperience: !!item.privateExperience,
    location: item.location && typeof item.location === 'object' ? item.location : null,
    locationLabel: item.locationLabel
      || (item.location && item.location.name)
      || null,
    videos: Array.isArray(item.videos) ? item.videos : [],
    seasonalOpeningHours: Array.isArray(item.seasonalOpeningHours) ? item.seasonalOpeningHours : [],
    seasonalOpeningHoursLabels: Array.isArray(item.seasonalOpeningHoursLabels)
      ? item.seasonalOpeningHoursLabels
      : [],
    ticketInfoHtml: typeof item.ticketInfoHtml === 'string' ? item.ticketInfoHtml : '',
  };
