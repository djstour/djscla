...
const { htmlLinksToMarkdown } = require('./vendorHtmlLinks');

function stripHtml(html) {
  return (html || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
}

/** Preserve block boundaries as blank lines for OpenAI (description / long HTML). */
function htmlToTranslatableText(html) {
  if (!html) return '';
  let out = String(html);
  out = htmlLinksToMarkdown(out);
  out = out.replace(/<\s*br\s*\/?>/gi, '\n');
  out = out.replace(/<\s*\/\s*(p|div|h[1-6]|li|tr)\s*>/gi, '\n\n');
  out = out.replace(/<[^>]+>/g, ' ');
  out = out.replace(/[^\S\n]+/g, ' ');
  out = out.replace(/ *\n */g, '\n');
  out = out.replace(/\n{3,}/g, '\n\n');
...
function extractTranslatableFields(activity) {
  ...
  const desc = htmlToTranslatableText(activity.description);
  const sum = stripHtml(activity.summary);
  if (desc && desc !== sum) add('description', desc);
  else if (desc && !sum) add('summary', desc);
  add('includedHtml', htmlToTranslatableText(activity.includedHtml));
  add('excludedHtml', htmlToTranslatableText(activity.excludedHtml));
  add('requirementsHtml', htmlToTranslatableText(activity.requirementsHtml));
  add('attentionHtml', htmlToTranslatableText(activity.attentionHtml));
  add('cancellationPolicyTitle', stripHtml(activity.cancellationPolicyTitle));
  add('cancellationPolicyHtml', htmlToTranslatableText(activity.cancellationPolicyHtml));
  add('ticketInfoHtml', htmlToTranslatableText(activity.ticketInfoHtml));
...
function fieldPriority(fieldPath) {
  ...
  if (fieldPath === 'ticketInfoHtml') return 8;
...
module.exports = {
  stripHtml,
  htmlToTranslatableText,
  sourceHash,
  extractTranslatableFields,
  sortTranslatableFields,
  fieldPriority,
};
