...
const { enrichActivityTicketInfo } = require('./bokunCustomFieldsV1Fallback');
const { mapV2AvailabilityList } = require('./mapV2Availability');
const { mergeV1AvailabilityPricing } = require('./bokunAvailabilityV1Fallback');
...
async function getActivityAvailabilities(id, { start, end = start, uiLang } = {}) {
  ...
  const mapped = mapV2AvailabilityList(slots, { startTimes: components.startTimes });
  return mergeV1AvailabilityPricing(id, mapped, { start, end });
}
