 ...
 *
 *   • Single experience  → `/online-sales/experience/{activityId}?date=…`
 *     Public booking engine (not `/bookings/create`, which is staff extranet).
 *
 *   • Multi-item cart    → `/online-sales/cart?cart=base64({…})`
 *
...
function buildSingleExperienceUrl(item, { contactPrefill, lang } = {}) {
  ...
  return `${host}${base}/experience/${encodeURIComponent(item.activityId)}${qs ? `?${qs}` : ''}`;
}
...
function buildCartUrl(items, { contactPrefill, lang } = {}) {
  ...
  if (lang) params.set('lang', lang === 'hant' ? 'zh-TW' : lang === 'hans' ? 'zh-CN' : 'en');
  const base = getShopBasePath();
  return `${host}${base}/cart?${params.toString()}`;
}
