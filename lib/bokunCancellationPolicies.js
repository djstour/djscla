...
const { fetchCancellationPolicyFromV1ActivityJson } = require('./bokunCancellationV1Fallback');
...
async function enrichActivityCancellationPolicy(activity) {
  ...
  try {
    ...
    if (resolved) {
      activity.cancellationPolicy = resolved;
      return activity;
    }

    const experienceId = activity.id ?? activity.experienceId;
    if (experienceId != null) {
      const fromV1 = await fetchCancellationPolicyFromV1ActivityJson(experienceId);
      if (fromV1 && (fromV1.title || (fromV1.penaltyRules && fromV1.penaltyRules.length))) {
        activity.cancellationPolicy = fromV1;
        return activity;
      }
    }

    console.warn(
      '[bokunCancellationPolicies] policy',
      policyId,
      'not resolved via v2 list or v1 activity.json fallback',
    );
  } catch (err) {
