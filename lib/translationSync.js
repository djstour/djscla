...
const { translateField, getTranslationModel } = require('./openaiTranslate');
...
async function needsTranslation(activityId, fieldPath, lang, sourceHash, force, forceMarker) {
  ...
  if (forceMarker && existing.meta && existing.meta.forceMarker === forceMarker) return false;
  return true;
}

async function activityNeedsAnyTranslation(activity, langs, force, forceMarker) {
  const activityId = String(activity.id);
  const fields = extractTranslatableFields(activity);
  for (const field of fields) {
    for (const lang of langs) {
      // eslint-disable-next-line no-await-in-loop
      if (await needsTranslation(activityId, field.fieldPath, lang, field.sourceHash, force, forceMarker)) {
        return true;
      }
    }
  }
 ...
 */
async function findActivitiesNeedingSync(activities, {
  langs = DEFAULT_LANGS,
  force = false,
  forceMarker = null,
  maxActivities = 5,
  ...
  for (const activity of activities) {
    // eslint-disable-next-line no-await-in-loop
    if (await activityNeedsAnyTranslation(activity, langs, force, forceMarker)) {
...
async function syncActivityTranslations(activity, opts = {}) {
  ...
  outer: for (const field of fields) {
    for (const lang of langs) {
      ...
      try {
        const should = await needsTranslation(
          activityId,
          field.fieldPath,
          lang,
          field.sourceHash,
  ...
  if (upsertRows.length) {
    ...
    try {
      ...
      await persistTranslationDisplay(activityId, activity);
    } catch (persistErr) {
      console.warn('[translationSync] persistTranslationDisplay', activityId, persistErr.message || persistErr);
    }
  }

  stats.complete = !(await activityNeedsAnyTranslation(activity, langs, force, forceMarker));
  return stats;
}

/**
 * @param {{ activityIds?: number[], limit?: number, langs?: string[], force?: boolean, forceMarker?: string, uiLang?: string }} opts
 */
...
async function runAutoTranslationSync(opts = {}) {
  ...
  const queue = await scanTranslationQueue({
    maxScan,
    pendingLimit: Math.max(maxActivities * 4, 40),
    langs,
  });
  ...
  const pending = [];
  if (force) {
    const need = await findActivitiesNeedingSync(catalog, { langs, force, forceMarker, maxActivities });
    pending.push(...need);
  } else {
    const priorityOrder = new Map(
      (queue.pending || []).map((item, idx) => [String(item.bokunActivityId), idx]),
    );
    const candidates = (queue.pending || [])
      .map((item) => byId.get(String(item.bokunActivityId)))
      .filter(Boolean)
      .sort((a, b) => (
        (priorityOrder.get(String(a.id)) ?? 999) - (priorityOrder.get(String(b.id)) ?? 999)
      ));
