...
/** Default quantity for the primary (Adult) pricing category in the booking sidebar. */
const DEFAULT_ADULT_PAX = 2;
...
function resolveActivityDuration(tour) {
  const vm = tour || {};
  const raw = vm.raw || vm;
  const text = String(vm.duration || vm.durationText || raw.durationText || '').trim();
  if (text && text !== '[object Object]') return text;

  const durObj = raw.duration;
  if (durObj && typeof durObj === 'object') {
    const fromObj = formatDurationLabel(durObj);
...
function resolveSupplierLabel(tour) {
  const vm = tour || {};
  const raw = vm.raw || vm;
  const vendor = vm.vendor || raw.vendor;
  const fromVendor = vendor && (vendor.titleOriginal || vendor.title || vendor.name);
  if (fromVendor && !isPlaceholderSupplierName(fromVendor)) return String(fromVendor).trim();
  if (vm.supplier && !isPlaceholderSupplierName(vm.supplier)) return String(vm.supplier).trim();
  return '';
}
...
function initPaxCounts(categories, legacy = {}) {
  ...
  if (adult) {
    counts[String(adult.id)] = legacyAdults > 0 ? legacyAdults : DEFAULT_ADULT_PAX;
  } else if (categories[0]) {
    counts[String(categories[0].id)] = legacyAdults > 0 ? legacyAdults : DEFAULT_ADULT_PAX;
  }
...
function resolveLivePaxCap({
  ...
  const knownCapacity = (n) => n != null && n !== '' && Number.isFinite(Number(n));
  const finiteCapacity = (n) => knownCapacity(n) && Number(n) >= 0;

  let live = null;

  const fromCheck = availabilityCheck?.availability?.capacityRemaining;
  if (finiteCapacity(fromCheck)) live = Number(fromCheck);
  ...
  if (live == null && startTimeId != null && startTimeId !== '' && Array.isArray(dayTimes)) {
    ...
    if (slot && finiteCapacity(slot.capacityRemaining)) live = Number(slot.capacityRemaining);
  }

  if (live == null && Array.isArray(dayTimes) && dayTimes.length === 1) {
    const only = dayTimes[0];
    if (only && finiteCapacity(only.capacityRemaining)) live = Number(only.capacityRemaining);
  }

  if (live == null && Array.isArray(dayTimes) && dayTimes.length > 1
    && (startTimeId == null || startTimeId === '')) {
    const slotCaps = dayTimes
      .filter((t) => !t.soldOut && finiteCapacity(t.capacityRemaining))
      .map((t) => Number(t.capacityRemaining));
    if (slotCaps.length) live = Math.min(...slotCaps);
  }

  if (live == null && dayInfo && finiteCapacity(dayInfo.capacityRemaining)) {
...
function unitPriceFromTour(tour, categoryId) {
  ...
  const fxRates = (typeof window !== 'undefined' && window.AuralisData && window.AuralisData._fxRates)
    ? window.AuralisData._fxRates
    : { USD: 1 };
  ...
  const tableMatches = table.filter((r) => String(r.categoryId) === id && Number(r.amount) > 0);
  if (tableMatches.length) {
    const amounts = tableMatches.map((r) => toUsd(r.amount, r.currency || r.sourceCurrency || 'USD'));
    return Math.min(...amounts);
  }

  const pricing = tour?.pricing || tour?.raw?.pricing || [];
  const prMatches = pricing.filter((p) => String(p.pricingCategoryId) === id && Number(p.amount) > 0);
  if (prMatches.length) {
    const amounts = prMatches.map((p) => toUsd(p.amount, p.currency || tour?.sourcePriceCurrency || 'USD'));
    return Math.min(...amounts);
  }
  return null;
}
