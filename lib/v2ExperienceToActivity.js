...
function mapPricingCategories(components) {
  ...
  return ids.map((id, idx) => ({
    id,
    title: idx === 0 ? 'Adult' : `Category ${id}`,
    fullTitle: idx === 0 ? 'Adult' : `Category ${id}`,
    defaultCategory: String(id) === String(defaultId),
  }));
}
...
function mapPricingFromRules(components, currency) {
  ...
  const defaultRateId = components.rates?.rates?.[0]?.id ?? components.rates?.[0]?.id ?? null;
  const rows = [];
  rules.forEach((rule) => {
    ...
    rows.push({
      ...
      activityRateId: rule.rate?.id ?? defaultRateId,
    });
  });
  return rows;
}
