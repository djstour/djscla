...
const { formatDurationLabel } = require('./catalogQuality');
...
function mapPickup(components, rates) {
  ...
  const pickupPlaceGroupIds = Array.isArray(mt.pickupPlaceGroupIds) ? mt.pickupPlaceGroupIds : [];
  const allPickupPlaceGroups = !!mt.allPickupPlaceGroups;
  const selectionAtHostedCheckout = pickupEnabled && !places.length
    && (pickupPlaceGroupIds.length > 0 || allPickupPlaceGroups);
  ...
  return {
    ...
    groups: pickupPlaceGroupIds.length,
    pickupPlaceGroupIds,
...
function mapProductLocation(loc) {
  if (!loc || typeof loc !== 'object') return null;
  const name = String(loc.name || loc.city || '').trim();
  if (!name && loc.latitude == null) return null;
  return {
    name,
    city: loc.city || null,
...
function formatSeasonalPeriod(period) {
  ...
  const sm = Number(period.startMonth);
  const sd = Number(period.startDay);
  const em = Number(period.endMonth);
  const ed = Number(period.endDay);
  if (![sm, sd, em, ed].every((n) => Number.isFinite(n) && n > 0)) return '';
  const sy = period.startYear ? ` ${period.startYear}` : '';
  const ey = period.endYear ? ` ${period.endYear}` : '';
  return `${sm}/${sd}${sy} – ${em}/${ed}${ey}`;
}

function mapSeasonalOpeningHours(list) {
  if (!Array.isArray(list)) return [];
  return list.map(formatSeasonalPeriod).filter(Boolean);
}

function mapVideos(videos) {
  return (videos || []).map((v, i) => ({
    id: v.id ?? i + 1,
    sourceUrl: v.sourceUrl || v.url || null,
...
function componentsToActivity(components, { experienceId, vendor } = {}) {
  ...
  const pricingCategories = mapPricingCategories(components);
  const pricing = mapPricingFromRules(components, currency);
  const defaultCatId = pricingCategories.find((c) => c.defaultCategory)?.id
    ?? pricingCategories[0]?.id;
  const defaultRow = defaultCatId != null
    ? pricing.find((p) => String(p.pricingCategoryId) === String(defaultCatId))
    : null;
  const fromRow = defaultRow || pricing.reduce((best, row) => {
    if (!row || !(Number(row.amount) > 0)) return best;
    if (!best || Number(row.amount) > Number(best.amount)) return row;
    return best;
  }, null);
  const fromAmount = fromRow && Number(fromRow.amount) > 0 ? Number(fromRow.amount) : null;
  ...
  return {
    ...
    durationText: formatDurationLabel(components.duration),
    ...
    isCombo: !!(components.combo && components.combo.isCombo),
    combo: components.combo || null,
    ...
    pricingCategories,
    pricing,
    nextDefaultPrice: fromAmount != null
      ? {
        amount: fromAmount,
        currency,
        pricingCategoryId: fromRow?.pricingCategoryId ?? defaultCatId,
      }
      : null,
    fromPrice: fromAmount != null ? { amount: fromAmount, currency } : null,
    ...
    pickupInfo,
    pickupPlaceGroupIds: pickupInfo.pickupPlaceGroupIds,
    rates,
    ...
    location: mapProductLocation(components.location),
    locationLabel: mapProductLocation(components.location)?.name || null,
    videos: mapVideos(components.videos),
    seasonalOpeningHours: Array.isArray(components.seasonalOpeningHours)
      ? components.seasonalOpeningHours
      : [],
    seasonalOpeningHoursLabels: mapSeasonalOpeningHours(components.seasonalOpeningHours),
    ticketInfoHtml: '',
    ...cutoffFields,
    apiVersion: 'v2',
  };
