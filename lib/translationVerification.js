...
function fieldTranslationLooksBroken(activity, fieldPath, translatedText) {
  ...
  if (expectedParts > 1 && parts.length !== expectedParts) return true;
  if (parts.some((p) => lineLooksBroken(p))) return true;

  if (fieldPath === 'title' && text.length > 120) return true;
  if (fieldPath === 'summary' && parts.length === 1 && lineLooksBroken(parts[0])) return true;
...
function evaluateTranslationTrust({
  ...
  if (adminApproved || (stored?.trusted === true && stored?.source === 'admin')) {
    return {
      ...base,
      trusted: true,
      source: 'admin',
...
function buildTranslationDisplaySnapshot(activity, overlay, langs = DEFAULT_LANGS) {
  ...
  langs.forEach((lang) => {
    out[lang] = evaluateTranslationTrust({ activity, overlay, lang });
  });
  return out;
}

/** Admin batch-approve helper — automated checks only, not yet admin-live. */
