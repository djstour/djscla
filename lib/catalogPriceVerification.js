...
const BLOCKED_REASONS = new Set([
  ...
  'implausible_catalog',
  'implausible_for_duration',
  'v2_components_not_for_display',
]);

/** Multi-day tours cannot show a sub-$25/day catalog "from" — v2 often stores commission slices. */
function catalogPriceTooLowForDuration(activity) {
  const mins = Number(activity && activity.durationMinutes);
  const max = catalogMaxUsd(activity);
  if (!Number.isFinite(mins) || mins < 1440 || !Number.isFinite(max) || max <= 0) return false;
  const days = mins / 1440;
  const perDay = max / days;
  return perDay < 25;
}
...
function evaluateCatalogPriceTrust({ activity, commissionPct = null }) {
  ...
  if (catalogPriceTooLowForDuration(activity)) {
    return {
      ...base,
      trusted: false,
      source: 'v2_components',
      reason: 'implausible_for_duration',
      message: 'v2 catalog 牌價相對行程天數過低（疑似佣金或非售價）',
    };
  }
...
/** Public gate — only show prices with a fresh trusted v2 audit from catalog sync. */
...
/**
 * Attach a trusted priceDisplay when catalog sync omitted it (e.g. list re-upsert
 * replaced bokun_payload). Keeps public UI aligned with stored v2 pricing rows.
 */
function hydrateCatalogPriceDisplay(activity) {
  if (!activity || typeof activity !== 'object') return activity;
  if (pricingLooksMislabeled(activity)) return activity;
  if (isPriceDisplayFresh(activity.priceDisplay)) return activity;
  if (!hasUsablePrice(activity)) return activity;

  const priceDisplay = evaluateCatalogPriceTrust({ activity, commissionPct: null });
  return {
    ...activity,
    priceDisplay,
