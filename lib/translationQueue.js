...
/** Automated QA failed (broken copy) — not missing/stale, not ready, not live. */
function isChecksFailedLocale(ap) {
  if (!ap || ap.live || ap.readyToApprove) return false;
  if ((ap.missing || 0) > 0 || (ap.stale || 0) > 0) return false;
  return true;
}

function formatBlockedHint(approval) {
  const parts = [];
  DEFAULT_LANGS.forEach((lang) => {
    const ap = approval && approval[lang];
    if (!isChecksFailedLocale(ap)) return;
    const tag = lang === 'hant' ? 'TC' : 'SC';
    const fields = (ap.brokenFields && ap.brokenFields.length)
      ? ap.brokenFields.join(', ')
      : (ap.message || ap.reason || 'checks failed');
    parts.push(`${tag}: ${fields}`);
  });
  return parts.join(' · ');
}
...
async function scanTranslationQueue({
  ...
  const approvalSummary = {
    ...
    blockedHant: assessments.filter((a) => isChecksFailedLocale(a.approval.hant)).length,
    blockedHans: assessments.filter((a) => isChecksFailedLocale(a.approval.hans)).length,
    blockedActivities: assessments.filter((a) => (
      isChecksFailedLocale(a.approval.hant) || isChecksFailedLocale(a.approval.hans)
    )).length,
  };
  ...
  const blockedQueue = assessments
    .filter((a) => (
      isChecksFailedLocale(a.approval.hant) || isChecksFailedLocale(a.approval.hans)
    ))
    .sort((a, b) => {
      const score = (x) => (
        (isChecksFailedLocale(x.approval.hant) ? 1 : 0)
        + (isChecksFailedLocale(x.approval.hans) ? 1 : 0)
        + (x.approval.hant.broken || 0)
        + (x.approval.hans.broken || 0)
      );
      return score(b) - score(a) || b.priority - a.priority;
    })
    .slice(0, Math.min(Math.max(blockedLimit, 1), 200))
    .map((a) => ({
      bokunActivityId: a.bokunActivityId,
      title: a.title,
      approval: a.approval,
      blockHint: formatBlockedHint(a.approval),
    }));
