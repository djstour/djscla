...
const { getSupabaseConfig, restHeaders, supabaseRestFetch } = require('./supabase');

const OVERLAY_ID_CHUNK = 50;
...
async function fetchActivityTranslationsOverlay(activityIds, langs = ['hant', 'hans']) {
  ...
  for (let i = 0; i < ids.length; i += OVERLAY_ID_CHUNK) {
    const slice = ids.slice(i, i + OVERLAY_ID_CHUNK);
    let offset = 0;
    // Paginate — 50 activities × fields × langs can exceed PostgREST default 1000 rows.
    // eslint-disable-next-line no-constant-condition
    while (true) {
      const params = new URLSearchParams();
      params.set('entity_type', 'eq.activity');
      params.set('entity_id', `in.(${slice.join(',')})`);
      params.set('or', `(${langList})`);
      params.set('select', 'entity_id,field_path,lang,text,meta');
      params.set('order', 'entity_id.asc,field_path.asc,lang.asc');
      params.set('limit', String(OVERLAY_PAGE_SIZE));
      params.set('offset', String(offset));

      // eslint-disable-next-line no-await-in-loop
      const batch = await supabaseRestFetch(`/rest/v1/translations?${params}`);
      const rows = Array.isArray(batch) ? batch : [];
      allRows.push(...rows);
      if (rows.length < OVERLAY_PAGE_SIZE) break;
      offset += OVERLAY_PAGE_SIZE;
    }
  }

  return rowsToActivityOverlay(allRows);
}
