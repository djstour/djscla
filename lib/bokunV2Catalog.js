...
/** List / catalog sync — omit MARKETPLACE (Bókun returns 500 for that component on many products). */
...
function buildChannelMeta({ activities, entries, membership, vendors, extra }) {
  const contractExperienceIds = (entries || []).map((e) => String(e.experienceId));
  const contractMembership = (entries || []).map((e) => ({
    bokunVendorId: e.supplierId != null ? String(e.supplierId) : '0',
    bokunActivityId: String(e.experienceId),
  }));
  const componentsLoaded = activities.length;
  const componentsRequested = extra.componentsRequested != null
    ? extra.componentsRequested
    : entries.length;
  return {
    page: 1,
    ...
    contractExperienceIds,
    contractMembership,
    componentsRequested,
    componentsLoaded,
    componentsFetchFailed: Math.max(0, componentsRequested - componentsLoaded),
...
async function fetchChannelCatalogForSync({
  ...
  const { activities: normalized, fetchMs, requested } = await fetchEntriesNormalized(slice, {
    concurrency,
    componentTypes,
    label: 'sync',
  });
  const activities = await applyQuoteCurrencyAsync(normalized, getQuoteCurrency());
