...
const { searchActivities } = require('./bokun');
...
async function probeBokun() {
  ...
  try {
    const res = await searchActivities({ uiLang: 'en', page: 1, pageSize: 1 });
    const total = res && res.totalHits != null ? res.totalHits : (res && res.activities && res.activities.length);
    return {
      id: 'bokun',
      label: 'Bókun contract search',
      status: 'ok',
      latencyMs: Date.now() - started,
      message: total != null ? `OK · channel hits ~${total}` : 'OK',
    };
  } catch (err) {
    return {
      id: 'bokun',
