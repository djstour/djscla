...
const { auditActivityPayload, hasPlausiblePrice } = require('./catalogQuality');
...
async function syncActivityDetails(needsDetail) {
  if (!needsDetail.size) {
    return {
      updated: 0, errors: 0, priceWarnings: 0, pickupHostedOnly: 0,
    };
  }
  ...
  for (const bokunId of needsDetail) {
    try {
      ...
      const audit = auditActivityPayload(mergedPayload);
      if (audit.priceImplausible) priceWarnings += 1;
  ...
  return {
    updated, errors, priceWarnings, pickupHostedOnly,
  };
}
