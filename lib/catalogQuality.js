...
const MIN_ADMISSION_DISPLAY_USD = Number(process.env.CATALOG_MIN_ADMISSION_USD) || 3;

/** Product types that legitimately sell below the tour/day-trip floor (e.g. Kerið admission). */
const LOW_PRICE_ACTIVITY_TYPES = new Set(['ATTRACTION']);
...
/** Default (usually Adult) pricing category — matches Bókun booking UI "From" basis. */
function defaultPricingCategoryId(activity) {
  const cats = Array.isArray(activity && activity.pricingCategories)
    ? activity.pricingCategories
    : [];
  const def = cats.find((c) => c && c.defaultCategory)
    || cats.find((c) => c && c.ticketCategory === 'ADULT')
    || cats[0];
  return def && def.id != null ? def.id : null;
}

/**
 * Catalog "from" row — default/adult category, not cheapest child/infant tier.
 * @returns {{ amount: number, currency: string, pricingCategoryId?: * }|null}
 */
function catalogFromPriceAmount(activity) {
  if (!activity || typeof activity !== 'object') return null;

  const currency = activity.sourceCurrency || activity.currency || activity.defaultCurrency || 'USD';
  const catId = defaultPricingCategoryId(activity);
  const rows = Array.isArray(activity.pricing) ? activity.pricing : [];

  if (catId != null) {
    const row = rows.find((r) => String(r && r.pricingCategoryId) === String(catId));
    const amount = Number(row && row.amount);
    if (Number.isFinite(amount) && amount > 0) {
      return {
        amount,
        currency: (row.currency || currency || 'USD').toUpperCase(),
        pricingCategoryId: catId,
      };
    }
  }

  if (activity.fromPrice && typeof activity.fromPrice === 'object') {
    const amount = Number(activity.fromPrice.amount);
    if (Number.isFinite(amount) && amount > 0) {
      return {
        amount,
        currency: (activity.fromPrice.currency || currency || 'USD').toUpperCase(),
      };
    }
  }

  const next = activity.nextDefaultPrice;
  if (next && Number.isFinite(Number(next.amount)) && Number(next.amount) > 0) {
    return {
      amount: Number(next.amount),
      currency: (next.currency || currency || 'USD').toUpperCase(),
      pricingCategoryId: next.pricingCategoryId,
    };
  }
...
function catalogFromPriceUsd(activity) {
  const row = catalogFromPriceAmount(activity);
  return row && Number.isFinite(row.amount) && row.amount > 0 ? row.amount : null;
}
...
function isAdminTrustedCatalogPrice(activity) {
  const pd = activity && activity.priceDisplay;
  if (!pd || pd.trusted !== true) return false;
  return pd.source === 'admin_override';
}

/** Attraction/admission tickets — catalog max can sit below the tour sanity floor. */
function isExpectedLowCatalogPrice(activity) {
  if (!activity || !hasUsablePrice(activity)) return false;
  const type = String(activity.activityType || '').toUpperCase();
  if (!LOW_PRICE_ACTIVITY_TYPES.has(type)) return false;
  const maxUsd = Math.max(...collectDisplayAmounts(activity));
  return Number.isFinite(maxUsd) && maxUsd >= MIN_ADMISSION_DISPLAY_USD;
}

function isCatalogPriceImplausible(activity) {
  if (!hasUsablePrice(activity)) return false;
  if (isAdminTrustedCatalogPrice(activity)) return false;
  if (isExpectedLowCatalogPrice(activity)) return false;
  const maxUsd = Math.max(...collectDisplayAmounts(activity));
  return maxUsd < MIN_PLAUSIBLE_DISPLAY_PRICE;
}

function hasPlausiblePrice(activity) {
  if (!hasUsablePrice(activity)) return false;
  if (isExpectedLowCatalogPrice(activity)) return true;
...
function auditActivityPayload(activity, { detailSyncedAt } = {}) {
  ...
  return {
    ...
    priceImplausible: isCatalogPriceImplausible(activity),
    priceVerified,
    priceUntrusted,
...
module.exports = {
  ...
  MIN_ADMISSION_DISPLAY_USD,
  LOW_PRICE_ACTIVITY_TYPES,
  collectDisplayAmounts,
  defaultPricingCategoryId,
  catalogFromPriceAmount,
  catalogFromPriceUsd,
  isAdminTrustedCatalogPrice,
  isExpectedLowCatalogPrice,
  isCatalogPriceImplausible,
  formatDurationLabel,
