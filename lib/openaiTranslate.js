...
function systemPrompt() {
  return `You are a localisation editor for Auralis, a premium Iceland OTA for Mandarin-speaking adventurers (primary market: Taiwan, secondary: Mainland China and Singapore).
 ...
 */
function lengthOk(source, translation, lang) {
  ...
  if (isCjk) {
    if (s <= 150) return ratio >= 0.25 && ratio <= 2.5;
    if (ratio >= 0.1 && ratio <= 2.5) return true;
...
Return JSON: { "translation": string, "notes": string|null }`;
  ...
  if (!lengthOk(source, translation, lang)) {
