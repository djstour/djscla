...
function lengthOk(source, translation, lang, fieldPath = '') {
  ...
  // Bókun sometimes stores a place name (e.g. "KERIÐ") as cancellationPolicyTitle —
  // OpenAI correctly keeps the Latin; accept unchanged proper nouns.
  if (shortLabel && isCjk) {
    if (translation.trim().toLowerCase() === source.trim().toLowerCase()) return true;
    return /[\u4e00-\u9fff]/.test(translation);
  }

  if (isCjk) {
    if (s <= 150) {
      // Brief notices compress heavily in CJK (e.g. attentionHtml, policy titles).
      return ratio >= 0.18 && ratio <= 3;
    }
