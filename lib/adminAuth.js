...
/** Non-blocking admin check for translation preview bypass on public read APIs. */
...
function isTranslationPreviewQuery(req) {
  return !!(req.query && (req.query.translationPreview === '1' || req.query.preview === '1'));
}

function isTranslationPreviewRequest(req) {
  return isTranslationPreviewQuery(req) && isAdminAuthorized(req);
}
