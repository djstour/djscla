...
(function () {
  const COPY = {
    ...
    translationVerifyPolicy: {
      hant: '繁中與簡中須分別核准後才會在對應語系前台上架。請先用「預覽繁中／預覽簡中」在完整行程頁核對，再按「核准繁中」或「核准簡中」。',
    ...
    previewTranslationHansBtn: { hant: '預覽簡中', hans: '预览简中', en: 'Preview SC' },
    ...
    previewTranslationHansHint: {
      hant: '在新分頁以完整前台 UI 預覽簡中（須已登入管理後台）',
    ...
    approvalBothLangsOnly: { hant: '只顯示繁中+簡中都可核准', hans: '只显示繁中+简中都可核准', en: 'Both locales ready only' },
    batchApproveHantBtn: { hant: '批量核准繁中', hans: '批量核准繁中', en: 'Batch approve TC' },
    batchApproveHansBtn: { hant: '批量核准簡中', hans: '批量核准简中', en: 'Batch approve SC' },
    ...
    thApprovalHans: { hant: '簡中', hans: '简中', en: 'SC' },
    ...
    approvalBlocked: { hant: '檢查未過', hans: '检查未过', en: 'Checks failed' },
    approvalSummaryApproved: { hant: '已核准', hans: '已核准', en: 'Approved' },
    approvalSummaryReady: { hant: '可核准', hans: '可核准', en: 'Ready' },
    approvalSummaryBlocked: { hant: '檢查未過', hans: '检查未过', en: 'Checks failed' },
    approvalBothReadyShort: { hant: '雙語', hans: '双语', en: 'Both' },
    blockedQueueTitle: { hant: '檢查未過（無法核准）', hans: '检查未过（无法核准）', en: 'Checks failed (cannot approve)' },
    blockedQueueSub: {
      hant: '譯文字段已存在，但自動結構檢查未通過（列表項數不符、句尾斷裂等）。請按「翻譯此行程」重翻；通過後才會出現在「待核准」。已核准 {liveHant}/{total} 繁中、{liveHans}/{total} 簡中。',
      hans: '译文字段已存在，但自动结构检查未通过（列表项数不符、句尾断裂等）。请按「翻译此行程」重翻；通过后才会出现在「待核准」。已核准 {liveHans}/{total} 简中、{liveHant}/{total} 繁中。',
      en: 'Translations exist but failed automated structure checks (list mismatch, broken fragments). Re-run Translate activity; then they move to Ready to approve. Live: {liveHant}/{total} TC, {liveHans}/{total} SC.',
    },
    thBlockReason: { hant: '未過原因', hans: '未过原因', en: 'Failed checks' },
    localeTagHant: { hant: '繁', hans: '繁', en: 'TC' },
    localeTagHans: { hant: '簡', hans: '简', en: 'SC' },
    approvalReadyCount: {
      hant: '可核准：繁中 {hant} · 簡中 {hans} · 雙語 {both}',
    ...
    batchApproveResultHans: {
      hant: '已核准 {ok} 筆{failed} · 已上線簡中 {liveBefore} → {liveAfter}（+{delta}）',
    ...
    batchApproveAllReadyHans: { hant: '核准全部可核准簡中', hans: '核准全部可核准简中', en: 'Approve all ready SC' },
    approvalVerifyHint: {
    ...
    batchRetranslateBlockedBtn: { hant: '批量重翻選取', hans: '批量重翻选取', en: 'Retranslate selected' },
    batchRetranslateAllBlockedBtn: { hant: '重翻本頁全部', hans: '重翻本页全部', en: 'Retranslate all on page' },
    batchRetranslateBlockedHint: {
      hant: '強制重翻繁中／簡中欄位以修復結構檢查；每筆約 30–90 秒，請保持此分頁開啟。',
      hans: '强制重翻繁中／简中字段以修复结构检查；每笔约 30–90 秒，请保持此分页开启。',
      en: 'Force re-translate TC/SC fields to fix structure checks (~30–90s each). Keep this tab open.',
    },
    batchRetranslateBlockedProgress: {
      hant: '重翻中 {done}/{total} · #{id}',
      hans: '重翻中 {done}/{total} · #{id}',
      en: 'Retranslating {done}/{total} · #{id}',
    },
    batchRetranslateBlockedResult: {
      hant: '已重翻 {ok} 筆{failed}',
      hans: '已重翻 {ok} 笔{failed}',
      en: 'Retranslated {ok}{failed}',
    },
    batchRetranslateBusyBtn: { hant: '重翻中…', hans: '重翻中…', en: 'Retranslating…' },
    retranslateActivityProgress: {
      hant: '重翻行程 #{id}',
      hans: '重翻行程 #{id}',
      en: 'Retranslating #{id}',
    },
    retranslateActivityProgressHint: {
      hant: '已進行 {s} 秒 · 單筆通常 30–90 秒 · 請保持此分頁開啟',
      hans: '已进行 {s} 秒 · 单笔通常 30–90 秒 · 请保持此分页开启',
      en: 'Elapsed {s}s · one activity usually 30–90s · keep this tab open',
    },
    editTranslationBtn: { hant: '編輯譯文', hans: '编辑译文', en: 'Edit translations' },
    translationEditorTitle: { hant: '編輯譯文', hans: '编辑译文', en: 'Edit translations' },
    translationEditorSub: {
      hant: '直接修改 Supabase 譯文，不呼叫 OpenAI。儲存後會重新跑自動檢查；通過後可核准上架。',
      hans: '直接修改 Supabase 译文，不调用 OpenAI。保存后会重新跑自动检查；通过后可核准上架。',
      en: 'Edit Supabase overlay only — no OpenAI. Saves re-run automated checks; approve when ready.',
    },
    translationEditorSource: { hant: '英文原文', hans: '英文原文', en: 'English source' },
    translationEditorFieldBroken: { hant: '檢查未過', hans: '检查未过', en: 'Failed checks' },
    translationEditorSave: { hant: '儲存譯文', hans: '保存译文', en: 'Save translations' },
    translationEditorNoChanges: { hant: '沒有變更可儲存', hans: '没有变更可保存', en: 'No changes to save' },
    translationEditorSaved: {
      hant: '已儲存。繁中 {hantReady} · 簡中 {hansReady}',
      hans: '已保存。繁中 {hantReady} · 简中 {hansReady}',
      en: 'Saved. TC {hantReady} · SC {hansReady}',
    },
    translationEditorReadyLabel: { hant: '可核准', hans: '可核准', en: 'Ready' },
    translationEditorBlockedLabel: { hant: '仍 blocked', hans: '仍 blocked', en: 'Still blocked' },
