...
| 逐日 slot 價 | v2 availability DTO 無單價 | v1 `activity.json/{id}/availabilities` 補 `pricesByRate`（`lib/bokunAvailabilityV1Fallback.js`，`BOKUN_AVAILABILITY_V1_FALLBACK=0` 關閉）；**不用** v2 components 牌價作 fallback |
